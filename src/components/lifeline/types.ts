...
/**
 * A diary volume sitting on the rail. When an event carries one, the
 * plain line of text is replaced by a closed book that opens into the
 * reader on click. This is the one addition to the stock lifeline
 * contract — everything else is upstream.
 */
export interface LifelineVolume {
  slug: string
  title: string
  /** Stamped across the cover, e.g. "11 · FEB · 2024". */
  dateLabel: string
  standfirst?: string
  commodity?: string
  place?: string
  /** Which of the cover treatments to use — seeded from the slug. */
  variant?: number
}
