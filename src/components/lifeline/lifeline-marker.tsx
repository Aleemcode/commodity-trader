...
import {
  ...
  getLifelineEventKey,
  getLifelineEventVolume,
  LifelineEventText,
} from "./lifeline-event"
import { LifelineVolumeCard } from "./lifeline-volume"
...
>(function LifelineMarkerColumn(
  ...
  const volumeCount = marker.events.filter((event) =>
    getLifelineEventVolume(event),
  ).length
        ...
        <div className="flex w-full flex-col items-start text-left">
          ...
          <div className="relative w-full pb-10 text-[#9d8564] transition-colors duration-300 group-hover:text-[#f4e8d3]">
            ...
            >
              ...
              {/* More than one entry in a month sits side by side rather
                  than stacked: two volumes stacked run past the bottom
                  of the stage, and side by side is also how they would
                  actually lie on a desk. */}
              <div
                className={cn(
                  "min-h-[3.25rem]",
                  volumeCount > 1
              ...
              >
                {marker.events.map((event, index) => {
                  ...
                  const volume = getLifelineEventVolume(event)
